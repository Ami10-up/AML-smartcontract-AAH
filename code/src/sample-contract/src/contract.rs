...
use crate::state::{ADMIN, ORACLE_DATA, ORACLE_PUBKEY, ORACLE_PUBKEY_TYPE, parse_key_type, RISK_SCORES};
...
// adding risk score check before allowing Send in existing method
fn execute_send(
    deps: DepsMut,
    info: MessageInfo,
    recipient: String,
    ...
    // fetch risk score
    let risk_score = RISK_SCORES
        .may_load(deps.storage, &sender_addr)?
        .unwrap_or(1); // default baseline risk

    // define threshold
    let threshold = 5; // example: block if risk > 5
    if risk_score > threshold {
        return Err(StdError::generic_err(format!(
            "transaction blocked: risk score {} exceeds threshold {}",
...
// using existing msg to write risk score updates
fn execute_oracle_update(
    ...
    // Load oracle public key and type
    ...
    // Parse incoming data as JSON: { "address": "...", "risk_score": 4 }
    let parsed: serde_json::Value = serde_json::from_str(&data)
        .map_err(|_| StdError::generic_err("failed to parse JSON data"))?;

    let addr_str = parsed["address"]
        .as_str()
        .ok_or_else(|| StdError::generic_err("missing address field"))?;
    let risk_score = parsed["risk_score"]
        .as_u64()
        .ok_or_else(|| StdError::generic_err("missing risk_score field"))? as u8;

    let addr = deps.api.addr_validate(addr_str)?;

    // Save the risk score in contract state
    RISK_SCORES.save(deps.storage, &addr, &risk_score)?;

    // Optional: save raw data if you still want ORACLE_DATA
    ORACLE_DATA.save(deps.storage, &data)?;

    // Emit event for transparency
    let event = Event::new("oracle_risk_update")
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    use crate::msg::{AdminResponse, OracleDataResponse, OraclePubkeyResponse, RiskScoreResponse};
    ...
    match msg {
        ...
        QueryMsg::GetRiskScore { address } => {
            let addr = deps.api.addr_validate(&address)?;
            let score = RISK_SCORES.may_load(deps.storage, &addr)?;
            to_binary(&RiskScoreResponse {
